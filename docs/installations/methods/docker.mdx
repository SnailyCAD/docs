...
:::tip
**Please take a minute and read the [in-depth document for each item here](/docs/guides/env/reference)**
:::
...
#### 2.2 Copying the .env file

After configuring the `.env` file, we must copy it to the Client and API. This is done by running the following command:

```bash
node scripts/copy-env.mjs --client --api
```
#### 2.2.1 Copying the .env file
 If the above command gave the Error 
...
Error [ERR_MODULE_NOT_FOUND]: Cannot find package 'dotenv' imported from /home/willow/snaily-cadv4/scripts/copy-env.mjs
