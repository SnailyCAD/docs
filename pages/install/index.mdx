...
- `CORS_ORIGIN_URL`: This is the URL to your site where the CAD is hosted, eg `https://cad.mysite.com` or use a wildcard to allow on all sites `*` (**very insecure!**)
...
*Last but not least..*, we must build the client's UI:
...
To stop the CAD, simply press `CTRL + C` once in the command prompt/terminal and wait for the services to stop.
